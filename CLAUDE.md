...
- **The ficlone move is copy-then-delete, not `rename`, for cross-filesystem/reflink cases** (`rename(2)` can't cross filesystems or mount boundaries). `do_ficlone()` (`ficlone.c`) clones to the waste, then unlinks the source *only after the clone succeeds*, and rolls back (unlinks the dest) if the source unlink fails — so the data is never gone from both places. `do_ficlone_dir()` (directories) has **no** clean rollback: a mid-way failure can leave a partial copy in the waste plus a partially-removed source (not data loss, but a mess — see deferred items).
...
- `do_ficlone_dir()` has no rollback for a partial directory clone (a mid-way failure leaves a partial copy + partially-removed source); the single-file path does roll back (task tracked)
